...
-- 7. VIEWS
CREATE OR REPLACE VIEW public.dashboard_agenda AS  SELECT id,
    user_id,
    judul,
    tanggal,
    jam,
...
CREATE OR REPLACE VIEW public.dashboard_pengumuman AS  SELECT id,
    user_id,
    judul,
    prioritas,
    tanggal_mulai,
...
CREATE OR REPLACE VIEW public.dashboard_today AS  SELECT j.id,
    j.user_id,
    k.nama_kelas,
    m.nama_mapel,
    j.hari,
...
CREATE OR REPLACE VIEW public.rekap_absensi_bulanan AS  SELECT user_id,
    murid_id,
...
CREATE OR REPLACE VIEW public.rekap_absensi_tahunan AS  SELECT user_id,
    murid_id,
    EXTRACT(year FROM tanggal) AS tahun,
...
CREATE OR REPLACE VIEW public.v_dashboard_presensi_ustaz_hari_ini AS  SELECT count(
        CASE
...
CREATE OR REPLACE VIEW public.view_rekap_nilai_murid AS  SELECT dn.murid_id,
    p.jenis,
...
CREATE POLICY "Akses mandiri absensi" ON public.absensi AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_absensi" ON public.absensi;
CREATE POLICY "delete_policy_absensi" ON public.absensi AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_absensi" ON public.absensi;
CREATE POLICY "insert_policy_absensi" ON public.absensi AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_absensi" ON public.absensi;
CREATE POLICY "select_policy_absensi" ON public.absensi AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_absensi" ON public.absensi;
CREATE POLICY "update_policy_absensi" ON public.absensi AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_own_absensi_audit" ON public.absensi_audit;
CREATE POLICY "delete_own_absensi_audit" ON public.absensi_audit AS PERMISSIVE FOR DELETE TO authenticated USING ((EXISTS ( SELECT 1
   FROM absensi
...
CREATE POLICY "insert_own_absensi_audit" ON public.absensi_audit AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((EXISTS ( SELECT 1
   FROM absensi
  WHERE ((absensi.id = absensi_audit.absensi_id) AND (absensi.user_id = uid())))));
DROP POLICY IF EXISTS "select_own_absensi_audit" ON public.absensi_audit;
CREATE POLICY "select_own_absensi_audit" ON public.absensi_audit AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
   FROM absensi
  WHERE ((absensi.id = absensi_audit.absensi_id) AND (absensi.user_id = uid())))));
DROP POLICY IF EXISTS "update_own_absensi_audit" ON public.absensi_audit;
CREATE POLICY "update_own_absensi_audit" ON public.absensi_audit AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1
   FROM absensi
  WHERE ((absensi.id = absensi_audit.absensi_id) AND (absensi.user_id = uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM absensi
  WHERE ((absensi.id = absensi_audit.absensi_id) AND (absensi.user_id = uid())))));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.agenda;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.agenda AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses mandiri agenda" ON public.agenda;
CREATE POLICY "Akses mandiri agenda" ON public.agenda AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "admin_all_agenda" ON public.agenda;
CREATE POLICY "admin_all_agenda" ON public.agenda AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "agenda_all" ON public.agenda;
CREATE POLICY "agenda_all" ON public.agenda AS PERMISSIVE FOR ALL TO public USING (is_admin());
DROP POLICY IF EXISTS "agenda_select" ON public.agenda;
CREATE POLICY "agenda_select" ON public.agenda AS PERMISSIVE FOR SELECT TO public USING ((uid() IS NOT NULL));
DROP POLICY IF EXISTS "delete_own_agenda" ON public.agenda;
CREATE POLICY "delete_own_agenda" ON public.agenda AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_agenda" ON public.agenda;
CREATE POLICY "insert_own_agenda" ON public.agenda AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_own_agenda" ON public.agenda;
CREATE POLICY "select_own_agenda" ON public.agenda AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_agenda" ON public.agenda;
CREATE POLICY "update_own_agenda" ON public.agenda AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_agenda_penting" ON public.agenda_penting;
CREATE POLICY "delete_policy_agenda_penting" ON public.agenda_penting AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_agenda_penting" ON public.agenda_penting;
CREATE POLICY "insert_policy_agenda_penting" ON public.agenda_penting AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_agenda_penting" ON public.agenda_penting;
CREATE POLICY "select_policy_agenda_penting" ON public.agenda_penting AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_agenda_penting" ON public.agenda_penting;
CREATE POLICY "update_policy_agenda_penting" ON public.agenda_penting AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_audit_trail_absensi" ON public.audit_trail_absensi;
CREATE POLICY "insert_audit_trail_absensi" ON public.audit_trail_absensi AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "select_audit_trail_absensi" ON public.audit_trail_absensi;
CREATE POLICY "select_audit_trail_absensi" ON public.audit_trail_absensi AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "delete_policy_bank_soal" ON public.bank_soal;
CREATE POLICY "delete_policy_bank_soal" ON public.bank_soal AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_bank_soal" ON public.bank_soal;
CREATE POLICY "insert_policy_bank_soal" ON public.bank_soal AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_bank_soal" ON public.bank_soal;
CREATE POLICY "select_policy_bank_soal" ON public.bank_soal AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_bank_soal" ON public.bank_soal;
CREATE POLICY "update_policy_bank_soal" ON public.bank_soal AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_policy_buku_saku" ON public.buku_saku;
CREATE POLICY "delete_policy_buku_saku" ON public.buku_saku AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_buku_saku" ON public.buku_saku;
CREATE POLICY "insert_policy_buku_saku" ON public.buku_saku AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_buku_saku" ON public.buku_saku;
CREATE POLICY "select_policy_buku_saku" ON public.buku_saku AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_buku_saku" ON public.buku_saku;
CREATE POLICY "update_policy_buku_saku" ON public.buku_saku AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_policy_capaian_hafalan" ON public.capaian_hafalan;
CREATE POLICY "delete_policy_capaian_hafalan" ON public.capaian_hafalan AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_capaian_hafalan" ON public.capaian_hafalan;
CREATE POLICY "insert_policy_capaian_hafalan" ON public.capaian_hafalan AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_capaian_hafalan" ON public.capaian_hafalan;
CREATE POLICY "select_policy_capaian_hafalan" ON public.capaian_hafalan AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_capaian_hafalan" ON public.capaian_hafalan;
CREATE POLICY "update_policy_capaian_hafalan" ON public.capaian_hafalan AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_policy_catatan_guru" ON public.catatan_guru;
CREATE POLICY "delete_policy_catatan_guru" ON public.catatan_guru AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_catatan_guru" ON public.catatan_guru;
CREATE POLICY "insert_policy_catatan_guru" ON public.catatan_guru AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_catatan_guru" ON public.catatan_guru;
CREATE POLICY "select_policy_catatan_guru" ON public.catatan_guru AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_catatan_guru" ON public.catatan_guru;
CREATE POLICY "update_policy_catatan_guru" ON public.catatan_guru AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_own_catatan_notif" ON public.catatan_guru_notifikasi;
CREATE POLICY "delete_own_catatan_notif" ON public.catatan_guru_notifikasi AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_catatan_notif" ON public.catatan_guru_notifikasi;
CREATE POLICY "insert_own_catatan_notif" ON public.catatan_guru_notifikasi AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_own_catatan_notif" ON public.catatan_guru_notifikasi;
CREATE POLICY "select_own_catatan_notif" ON public.catatan_guru_notifikasi AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_catatan_notif" ON public.catatan_guru_notifikasi;
CREATE POLICY "update_own_catatan_notif" ON public.catatan_guru_notifikasi AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_catatan_perilaku" ON public.catatan_perilaku;
CREATE POLICY "delete_policy_catatan_perilaku" ON public.catatan_perilaku AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_catatan_perilaku" ON public.catatan_perilaku;
CREATE POLICY "insert_policy_catatan_perilaku" ON public.catatan_perilaku AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_catatan_perilaku" ON public.catatan_perilaku;
CREATE POLICY "select_policy_catatan_perilaku" ON public.catatan_perilaku AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_catatan_perilaku" ON public.catatan_perilaku;
CREATE POLICY "update_policy_catatan_perilaku" ON public.catatan_perilaku AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.detail_nilai;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.detail_nilai AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses mandiri detail_nilai" ON public.detail_nilai;
CREATE POLICY "Akses mandiri detail_nilai" ON public.detail_nilai AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "admin_all_detail_nilai" ON public.detail_nilai;
CREATE POLICY "admin_all_detail_nilai" ON public.detail_nilai AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_detail_nilai_v2" ON public.detail_nilai;
CREATE POLICY "admin_all_detail_nilai_v2" ON public.detail_nilai AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "delete_own_detail_nilai" ON public.detail_nilai;
CREATE POLICY "delete_own_detail_nilai" ON public.detail_nilai AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_detail_nilai" ON public.detail_nilai;
CREATE POLICY "insert_own_detail_nilai" ON public.detail_nilai AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_all_detail_nilai" ON public.detail_nilai;
CREATE POLICY "select_all_detail_nilai" ON public.detail_nilai AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_all_detail_nilai_v2" ON public.detail_nilai;
CREATE POLICY "select_all_detail_nilai_v2" ON public.detail_nilai AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_own_detail_nilai" ON public.detail_nilai;
CREATE POLICY "select_own_detail_nilai" ON public.detail_nilai AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_detail_nilai" ON public.detail_nilai;
CREATE POLICY "update_own_detail_nilai" ON public.detail_nilai AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "insert_guru_pengganti" ON public.guru_pengganti;
CREATE POLICY "insert_guru_pengganti" ON public.guru_pengganti AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((EXISTS ( SELECT 1
   FROM profiles
...
CREATE POLICY "select_guru_pengganti" ON public.guru_pengganti AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "update_guru_pengganti" ON public.guru_pengganti;
CREATE POLICY "update_guru_pengganti" ON public.guru_pengganti AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = uid()) AND ((profiles.role)::text = ANY ((ARRAY['admin'::character varying, 'operator'::character varying])::text[])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = uid()) AND ((profiles.role)::text = ANY ((ARRAY['admin'::character varying, 'operator'::character varying])::text[]))))));
DROP POLICY IF EXISTS "delete_own_hari_belajar" ON public.hari_belajar;
CREATE POLICY "delete_own_hari_belajar" ON public.hari_belajar AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_hari_belajar" ON public.hari_belajar;
CREATE POLICY "insert_own_hari_belajar" ON public.hari_belajar AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_own_hari_belajar" ON public.hari_belajar;
CREATE POLICY "select_own_hari_belajar" ON public.hari_belajar AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_hari_belajar" ON public.hari_belajar;
CREATE POLICY "update_own_hari_belajar" ON public.hari_belajar AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.izin_mengajar;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.izin_mengajar AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "admin_all_izin_mengajar" ON public.izin_mengajar;
CREATE POLICY "admin_all_izin_mengajar" ON public.izin_mengajar AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "delete_own_izin_mengajar" ON public.izin_mengajar;
CREATE POLICY "delete_own_izin_mengajar" ON public.izin_mengajar AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_izin_mengajar" ON public.izin_mengajar;
CREATE POLICY "insert_own_izin_mengajar" ON public.izin_mengajar AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "izin_delete_sendiri" ON public.izin_mengajar;
CREATE POLICY "izin_delete_sendiri" ON public.izin_mengajar AS PERMISSIVE FOR DELETE TO public USING ((user_id = uid()));
DROP POLICY IF EXISTS "izin_insert_sendiri" ON public.izin_mengajar;
CREATE POLICY "izin_insert_sendiri" ON public.izin_mengajar AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id = uid()));
DROP POLICY IF EXISTS "izin_select_sendiri" ON public.izin_mengajar;
CREATE POLICY "izin_select_sendiri" ON public.izin_mengajar AS PERMISSIVE FOR SELECT TO public USING ((user_id = uid()));
DROP POLICY IF EXISTS "izin_update_sendiri" ON public.izin_mengajar;
CREATE POLICY "izin_update_sendiri" ON public.izin_mengajar AS PERMISSIVE FOR UPDATE TO public USING ((user_id = uid()));
DROP POLICY IF EXISTS "select_own_izin_mengajar" ON public.izin_mengajar;
CREATE POLICY "select_own_izin_mengajar" ON public.izin_mengajar AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_izin_mengajar" ON public.izin_mengajar;
CREATE POLICY "update_own_izin_mengajar" ON public.izin_mengajar AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "Akses mandiri jadwal" ON public.jadwal;
CREATE POLICY "Akses mandiri jadwal" ON public.jadwal AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_jadwal" ON public.jadwal;
CREATE POLICY "delete_policy_jadwal" ON public.jadwal AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_jadwal" ON public.jadwal;
CREATE POLICY "insert_policy_jadwal" ON public.jadwal AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "jadwal_insert_update_delete" ON public.jadwal;
CREATE POLICY "jadwal_insert_update_delete" ON public.jadwal AS PERMISSIVE FOR ALL TO public USING (is_admin());
DROP POLICY IF EXISTS "jadwal_select" ON public.jadwal;
CREATE POLICY "jadwal_select" ON public.jadwal AS PERMISSIVE FOR SELECT TO public USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_jadwal" ON public.jadwal;
CREATE POLICY "select_policy_jadwal" ON public.jadwal AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_jadwal" ON public.jadwal;
CREATE POLICY "update_policy_jadwal" ON public.jadwal AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_policy_jadwal_mengajar" ON public.jadwal_mengajar;
CREATE POLICY "delete_policy_jadwal_mengajar" ON public.jadwal_mengajar AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_jadwal_mengajar" ON public.jadwal_mengajar;
CREATE POLICY "insert_policy_jadwal_mengajar" ON public.jadwal_mengajar AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_jadwal_mengajar" ON public.jadwal_mengajar;
CREATE POLICY "select_policy_jadwal_mengajar" ON public.jadwal_mengajar AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_jadwal_mengajar" ON public.jadwal_mengajar;
CREATE POLICY "update_policy_jadwal_mengajar" ON public.jadwal_mengajar AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_own_jam_pelajaran" ON public.jam_pelajaran;
CREATE POLICY "delete_own_jam_pelajaran" ON public.jam_pelajaran AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_jam_pelajaran" ON public.jam_pelajaran;
CREATE POLICY "insert_own_jam_pelajaran" ON public.jam_pelajaran AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_own_jam_pelajaran" ON public.jam_pelajaran;
CREATE POLICY "select_own_jam_pelajaran" ON public.jam_pelajaran AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_jam_pelajaran" ON public.jam_pelajaran;
CREATE POLICY "update_own_jam_pelajaran" ON public.jam_pelajaran AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "Akses mandiri jurnal_kbm" ON public.jurnal_kbm;
CREATE POLICY "Akses mandiri jurnal_kbm" ON public.jurnal_kbm AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_jurnal_kbm" ON public.jurnal_kbm;
CREATE POLICY "delete_policy_jurnal_kbm" ON public.jurnal_kbm AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_jurnal_kbm" ON public.jurnal_kbm;
CREATE POLICY "insert_policy_jurnal_kbm" ON public.jurnal_kbm AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_jurnal_kbm" ON public.jurnal_kbm;
CREATE POLICY "select_policy_jurnal_kbm" ON public.jurnal_kbm AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_jurnal_kbm" ON public.jurnal_kbm;
CREATE POLICY "update_policy_jurnal_kbm" ON public.jurnal_kbm AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_policy_kbm_harian" ON public.kbm_harian;
CREATE POLICY "delete_policy_kbm_harian" ON public.kbm_harian AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_kbm_harian" ON public.kbm_harian;
CREATE POLICY "insert_policy_kbm_harian" ON public.kbm_harian AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_kbm_harian" ON public.kbm_harian;
CREATE POLICY "select_policy_kbm_harian" ON public.kbm_harian AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_kbm_harian" ON public.kbm_harian;
CREATE POLICY "update_policy_kbm_harian" ON public.kbm_harian AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.kelas;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.kelas AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses mandiri kelas" ON public.kelas;
CREATE POLICY "Akses mandiri kelas" ON public.kelas AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "Izinkan semua akun login membaca data kelas" ON public.kelas;
CREATE POLICY "Izinkan semua akun login membaca data kelas" ON public.kelas AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "admin_all_kelas" ON public.kelas;
CREATE POLICY "admin_all_kelas" ON public.kelas AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_kelas_v2" ON public.kelas;
CREATE POLICY "admin_all_kelas_v2" ON public.kelas AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "delete_own_kelas" ON public.kelas;
CREATE POLICY "delete_own_kelas" ON public.kelas AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_kelas" ON public.kelas;
CREATE POLICY "insert_own_kelas" ON public.kelas AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "kelas_insert_update_delete" ON public.kelas;
CREATE POLICY "kelas_insert_update_delete" ON public.kelas AS PERMISSIVE FOR ALL TO public USING (is_admin());
DROP POLICY IF EXISTS "kelas_select" ON public.kelas;
CREATE POLICY "kelas_select" ON public.kelas AS PERMISSIVE FOR SELECT TO public USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_all_kelas_v2" ON public.kelas;
CREATE POLICY "select_all_kelas_v2" ON public.kelas AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_own_kelas" ON public.kelas;
CREATE POLICY "select_own_kelas" ON public.kelas AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_kelas" ON public.kelas;
CREATE POLICY "update_own_kelas" ON public.kelas AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_lembaga_admin" ON public.lembaga;
CREATE POLICY "delete_lembaga_admin" ON public.lembaga AS PERMISSIVE FOR DELETE TO authenticated USING (true);
DROP POLICY IF EXISTS "insert_lembaga_admin" ON public.lembaga;
CREATE POLICY "insert_lembaga_admin" ON public.lembaga AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "select_lembaga_all" ON public.lembaga;
CREATE POLICY "select_lembaga_all" ON public.lembaga AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "update_lembaga_admin" ON public.lembaga;
CREATE POLICY "update_lembaga_admin" ON public.lembaga AS PERMISSIVE FOR UPDATE TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.mata_pelajaran;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.mata_pelajaran AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses mandiri mata_pelajaran" ON public.mata_pelajaran;
CREATE POLICY "Akses mandiri mata_pelajaran" ON public.mata_pelajaran AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "admin_all_mata_pelajaran" ON public.mata_pelajaran;
CREATE POLICY "admin_all_mata_pelajaran" ON public.mata_pelajaran AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_mata_pelajaran_v2" ON public.mata_pelajaran;
CREATE POLICY "admin_all_mata_pelajaran_v2" ON public.mata_pelajaran AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "delete_own_mata_pelajaran" ON public.mata_pelajaran;
CREATE POLICY "delete_own_mata_pelajaran" ON public.mata_pelajaran AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_mata_pelajaran" ON public.mata_pelajaran;
CREATE POLICY "insert_own_mata_pelajaran" ON public.mata_pelajaran AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "mapel_insert_update_delete" ON public.mata_pelajaran;
CREATE POLICY "mapel_insert_update_delete" ON public.mata_pelajaran AS PERMISSIVE FOR ALL TO public USING (is_admin());
DROP POLICY IF EXISTS "mapel_select" ON public.mata_pelajaran;
CREATE POLICY "mapel_select" ON public.mata_pelajaran AS PERMISSIVE FOR SELECT TO public USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_all_mata_pelajaran" ON public.mata_pelajaran;
CREATE POLICY "select_all_mata_pelajaran" ON public.mata_pelajaran AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_all_mata_pelajaran_v2" ON public.mata_pelajaran;
CREATE POLICY "select_all_mata_pelajaran_v2" ON public.mata_pelajaran AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_own_mata_pelajaran" ON public.mata_pelajaran;
CREATE POLICY "select_own_mata_pelajaran" ON public.mata_pelajaran AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_mata_pelajaran" ON public.mata_pelajaran;
CREATE POLICY "update_own_mata_pelajaran" ON public.mata_pelajaran AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "Akses mandiri materi" ON public.materi;
CREATE POLICY "Akses mandiri materi" ON public.materi AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_materi" ON public.materi;
CREATE POLICY "delete_policy_materi" ON public.materi AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_materi" ON public.materi;
CREATE POLICY "insert_policy_materi" ON public.materi AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_materi" ON public.materi;
CREATE POLICY "select_policy_materi" ON public.materi AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_materi" ON public.materi;
CREATE POLICY "update_policy_materi" ON public.materi AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "delete_policy_muhafadhoh" ON public.muhafadhoh;
CREATE POLICY "delete_policy_muhafadhoh" ON public.muhafadhoh AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_muhafadhoh" ON public.muhafadhoh;
CREATE POLICY "insert_policy_muhafadhoh" ON public.muhafadhoh AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_muhafadhoh" ON public.muhafadhoh;
CREATE POLICY "select_policy_muhafadhoh" ON public.muhafadhoh AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_muhafadhoh" ON public.muhafadhoh;
CREATE POLICY "update_policy_muhafadhoh" ON public.muhafadhoh AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.murid;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.murid AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses mandiri murid" ON public.murid;
CREATE POLICY "Akses mandiri murid" ON public.murid AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "admin_all_murid" ON public.murid;
CREATE POLICY "admin_all_murid" ON public.murid AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_murid_v2" ON public.murid;
CREATE POLICY "admin_all_murid_v2" ON public.murid AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "delete_own_murid" ON public.murid;
CREATE POLICY "delete_own_murid" ON public.murid AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_murid" ON public.murid;
CREATE POLICY "insert_own_murid" ON public.murid AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "murid_insert_update_delete" ON public.murid;
CREATE POLICY "murid_insert_update_delete" ON public.murid AS PERMISSIVE FOR ALL TO public USING (is_admin());
DROP POLICY IF EXISTS "murid_select" ON public.murid;
CREATE POLICY "murid_select" ON public.murid AS PERMISSIVE FOR SELECT TO public USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_all_murid" ON public.murid;
CREATE POLICY "select_all_murid" ON public.murid AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_all_murid_v2" ON public.murid;
CREATE POLICY "select_all_murid_v2" ON public.murid AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_own_murid" ON public.murid;
CREATE POLICY "select_own_murid" ON public.murid AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_murid" ON public.murid;
CREATE POLICY "update_own_murid" ON public.murid AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_nilai" ON public.nilai;
CREATE POLICY "delete_policy_nilai" ON public.nilai AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_nilai" ON public.nilai;
CREATE POLICY "insert_policy_nilai" ON public.nilai AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_nilai" ON public.nilai;
CREATE POLICY "select_policy_nilai" ON public.nilai AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_nilai" ON public.nilai;
CREATE POLICY "update_policy_nilai" ON public.nilai AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses mandiri notifikasi" ON public.notifikasi;
CREATE POLICY "Akses mandiri notifikasi" ON public.notifikasi AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_notifikasi" ON public.notifikasi;
CREATE POLICY "delete_policy_notifikasi" ON public.notifikasi AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_notifikasi" ON public.notifikasi;
CREATE POLICY "insert_policy_notifikasi" ON public.notifikasi AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_notifikasi" ON public.notifikasi;
CREATE POLICY "select_policy_notifikasi" ON public.notifikasi AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_notifikasi" ON public.notifikasi;
CREATE POLICY "update_policy_notifikasi" ON public.notifikasi AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.pengaturan;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.pengaturan AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses mandiri pengaturan" ON public.pengaturan;
CREATE POLICY "Akses mandiri pengaturan" ON public.pengaturan AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "admin_all_pengaturan" ON public.pengaturan;
CREATE POLICY "admin_all_pengaturan" ON public.pengaturan AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "delete_own_pengaturan" ON public.pengaturan;
CREATE POLICY "delete_own_pengaturan" ON public.pengaturan AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_pengaturan" ON public.pengaturan;
CREATE POLICY "insert_own_pengaturan" ON public.pengaturan AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_own_pengaturan" ON public.pengaturan;
CREATE POLICY "select_own_pengaturan" ON public.pengaturan AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_pengaturan" ON public.pengaturan;
CREATE POLICY "update_own_pengaturan" ON public.pengaturan AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "Akses mandiri pengumuman" ON public.pengumuman;
CREATE POLICY "Akses mandiri pengumuman" ON public.pengumuman AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_pengumuman" ON public.pengumuman;
CREATE POLICY "delete_policy_pengumuman" ON public.pengumuman AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_pengumuman" ON public.pengumuman;
CREATE POLICY "insert_policy_pengumuman" ON public.pengumuman AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "pengumuman_select" ON public.pengumuman;
CREATE POLICY "pengumuman_select" ON public.pengumuman AS PERMISSIVE FOR SELECT TO public USING ((uid() IS NOT NULL));
DROP POLICY IF EXISTS "select_policy_pengumuman" ON public.pengumuman;
CREATE POLICY "select_policy_pengumuman" ON public.pengumuman AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_pengumuman" ON public.pengumuman;
CREATE POLICY "update_policy_pengumuman" ON public.pengumuman AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses mandiri penilaian" ON public.penilaian;
CREATE POLICY "Akses mandiri penilaian" ON public.penilaian AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_penilaian" ON public.penilaian;
CREATE POLICY "delete_policy_penilaian" ON public.penilaian AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_penilaian" ON public.penilaian;
CREATE POLICY "insert_policy_penilaian" ON public.penilaian AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_penilaian" ON public.penilaian;
CREATE POLICY "select_policy_penilaian" ON public.penilaian AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_penilaian" ON public.penilaian;
CREATE POLICY "update_policy_penilaian" ON public.penilaian AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.presensi_guru;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.presensi_guru AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "Akses mandiri presensi_guru" ON public.presensi_guru;
CREATE POLICY "Akses mandiri presensi_guru" ON public.presensi_guru AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "admin_all_presensi_guru" ON public.presensi_guru;
CREATE POLICY "admin_all_presensi_guru" ON public.presensi_guru AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_presensi_guru_v2" ON public.presensi_guru;
CREATE POLICY "admin_all_presensi_guru_v2" ON public.presensi_guru AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "delete_own_presensi_guru" ON public.presensi_guru;
CREATE POLICY "delete_own_presensi_guru" ON public.presensi_guru AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_presensi_guru" ON public.presensi_guru;
CREATE POLICY "insert_own_presensi_guru" ON public.presensi_guru AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_all_presensi_guru_v2" ON public.presensi_guru;
CREATE POLICY "select_all_presensi_guru_v2" ON public.presensi_guru AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_own_presensi_guru" ON public.presensi_guru;
CREATE POLICY "select_own_presensi_guru" ON public.presensi_guru AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_presensi_guru" ON public.presensi_guru;
CREATE POLICY "update_own_presensi_guru" ON public.presensi_guru AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_presensi_ustaz" ON public.presensi_ustaz;
CREATE POLICY "insert_own_presensi_ustaz" ON public.presensi_ustaz AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = guru_id));
DROP POLICY IF EXISTS "select_own_presensi_ustaz" ON public.presensi_ustaz;
CREATE POLICY "select_own_presensi_ustaz" ON public.presensi_ustaz AS PERMISSIVE FOR SELECT TO authenticated USING (((uid() = guru_id) OR (EXISTS ( SELECT 1
   FROM profiles
...
CREATE POLICY "Admin bypass RLS for Insert" ON public.profiles AS PERMISSIVE FOR INSERT TO public WITH CHECK ((role() = 'authenticated'::text));
DROP POLICY IF EXISTS "Users can update own profile" ON public.profiles;
CREATE POLICY "Users can update own profile" ON public.profiles AS PERMISSIVE FOR UPDATE TO public USING ((uid() = id)) WITH CHECK ((uid() = id));
DROP POLICY IF EXISTS "Users can view all profiles" ON public.profiles;
CREATE POLICY "Users can view all profiles" ON public.profiles AS PERMISSIVE FOR SELECT TO public USING ((role() = 'authenticated'::text));
DROP POLICY IF EXISTS "admin_all_profiles" ON public.profiles;
CREATE POLICY "admin_all_profiles" ON public.profiles AS PERMISSIVE FOR ALL TO public USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_profiles_v2" ON public.profiles;
CREATE POLICY "admin_all_profiles_v2" ON public.profiles AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "select_all_profiles_v2" ON public.profiles;
CREATE POLICY "select_all_profiles_v2" ON public.profiles AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "delete_policy_rapor_final" ON public.rapor_final;
CREATE POLICY "delete_policy_rapor_final" ON public.rapor_final AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_rapor_final" ON public.rapor_final;
CREATE POLICY "insert_policy_rapor_final" ON public.rapor_final AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_rapor_final" ON public.rapor_final;
CREATE POLICY "select_policy_rapor_final" ON public.rapor_final AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_rapor_final" ON public.rapor_final;
CREATE POLICY "update_policy_rapor_final" ON public.rapor_final AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "admin_all_ruangan" ON public.ruangan;
CREATE POLICY "admin_all_ruangan" ON public.ruangan AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "select_all_ruangan" ON public.ruangan;
CREATE POLICY "select_all_ruangan" ON public.ruangan AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.semester;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.semester AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "admin_all_semester" ON public.semester;
CREATE POLICY "admin_all_semester" ON public.semester AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_semester_v2" ON public.semester;
CREATE POLICY "admin_all_semester_v2" ON public.semester AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "read_semester" ON public.semester;
CREATE POLICY "read_semester" ON public.semester AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_all_semester" ON public.semester;
CREATE POLICY "select_all_semester" ON public.semester AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_all_semester_v2" ON public.semester;
CREATE POLICY "select_all_semester_v2" ON public.semester AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "semester_all" ON public.semester;
CREATE POLICY "semester_all" ON public.semester AS PERMISSIVE FOR ALL TO public USING (is_admin());
DROP POLICY IF EXISTS "semester_select" ON public.semester;
CREATE POLICY "semester_select" ON public.semester AS PERMISSIVE FOR SELECT TO public USING ((uid() IS NOT NULL));
DROP POLICY IF EXISTS "Akses mandiri sikap" ON public.sikap;
CREATE POLICY "Akses mandiri sikap" ON public.sikap AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_sikap" ON public.sikap;
CREATE POLICY "delete_policy_sikap" ON public.sikap AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_sikap" ON public.sikap;
CREATE POLICY "insert_policy_sikap" ON public.sikap AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_sikap" ON public.sikap;
CREATE POLICY "select_policy_sikap" ON public.sikap AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_sikap" ON public.sikap;
CREATE POLICY "update_policy_sikap" ON public.sikap AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses mandiri soal" ON public.soal;
CREATE POLICY "Akses mandiri soal" ON public.soal AS PERMISSIVE FOR ALL TO public USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "delete_policy_soal" ON public.soal;
CREATE POLICY "delete_policy_soal" ON public.soal AS PERMISSIVE FOR DELETE TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "insert_policy_soal" ON public.soal;
CREATE POLICY "insert_policy_soal" ON public.soal AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "select_policy_soal" ON public.soal;
CREATE POLICY "select_policy_soal" ON public.soal AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "update_policy_soal" ON public.soal;
CREATE POLICY "update_policy_soal" ON public.soal AS PERMISSIVE FOR UPDATE TO authenticated USING ((is_admin() OR (user_id = uid()))) WITH CHECK ((is_admin() OR (user_id = uid())));
DROP POLICY IF EXISTS "Akses Penuh Admin Otomatis" ON public.tahun_ajaran;
CREATE POLICY "Akses Penuh Admin Otomatis" ON public.tahun_ajaran AS PERMISSIVE FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "admin_all_tahun_ajaran" ON public.tahun_ajaran;
CREATE POLICY "admin_all_tahun_ajaran" ON public.tahun_ajaran AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "admin_all_tahun_ajaran_v2" ON public.tahun_ajaran;
CREATE POLICY "admin_all_tahun_ajaran_v2" ON public.tahun_ajaran AS PERMISSIVE FOR ALL TO authenticated USING (is_admin()) WITH CHECK (is_admin());
DROP POLICY IF EXISTS "read_tahun_ajaran" ON public.tahun_ajaran;
CREATE POLICY "read_tahun_ajaran" ON public.tahun_ajaran AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_all_tahun_ajaran" ON public.tahun_ajaran;
CREATE POLICY "select_all_tahun_ajaran" ON public.tahun_ajaran AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "select_all_tahun_ajaran_v2" ON public.tahun_ajaran;
CREATE POLICY "select_all_tahun_ajaran_v2" ON public.tahun_ajaran AS PERMISSIVE FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "tahun_ajaran_all" ON public.tahun_ajaran;
CREATE POLICY "tahun_ajaran_all" ON public.tahun_ajaran AS PERMISSIVE FOR ALL TO public USING (is_admin());
DROP POLICY IF EXISTS "tahun_ajaran_select" ON public.tahun_ajaran;
CREATE POLICY "tahun_ajaran_select" ON public.tahun_ajaran AS PERMISSIVE FOR SELECT TO public USING ((uid() IS NOT NULL));
DROP POLICY IF EXISTS "delete_own_user_settings" ON public.user_settings;
CREATE POLICY "delete_own_user_settings" ON public.user_settings AS PERMISSIVE FOR DELETE TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "insert_own_user_settings" ON public.user_settings;
CREATE POLICY "insert_own_user_settings" ON public.user_settings AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((uid() = user_id));
DROP POLICY IF EXISTS "select_own_user_settings" ON public.user_settings;
CREATE POLICY "select_own_user_settings" ON public.user_settings AS PERMISSIVE FOR SELECT TO authenticated USING ((uid() = user_id));
DROP POLICY IF EXISTS "update_own_user_settings" ON public.user_settings;
CREATE POLICY "update_own_user_settings" ON public.user_settings AS PERMISSIVE FOR UPDATE TO authenticated USING ((uid() = user_id)) WITH CHECK ((uid() = user_id));
